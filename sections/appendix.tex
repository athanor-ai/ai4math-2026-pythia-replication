...
\item \texttt{Pythia.MeasureTheory.} \texttt{OptionalStoppingUnbounded},
  \texttt{Pythia.MeasureTheory.PathMeasureRN}: structural support
  for the Ville chain.
\end{itemize}
...
closure is \texttt{etavector\_eq\_sqrt\_two\_mul\_etahr} (the
$\sqrt{2}$ identity, also closed by Kimi K2.5 and Mistral Large 3 in
the wider drafter sweep). Source: dataset paper
...
with. \texttt{linarith} produces Farkas certificates,
\texttt{polyrith} routes polynomial-ideal goals via Sage with
kernel-checked reconstruction, \texttt{nlinarith} extends
\texttt{linarith} to a small non-linear fragment, \texttt{positivity}
discharges sign goals, \texttt{measurability} closes
$\sigma$-algebra-membership obligations, \texttt{gcongr} discharges
generalised-congruence monotonicity, and \texttt{bound} dispatches
monotone-composition goals via an attribute-driven rule database.
...
mirrors this template at small scale: external prover supplies the
verdict, in-kernel tactic supplies the proof term, kernel is the
final arbiter. We do not claim a full Lean-side reimplementation of
